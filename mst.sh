...
(cd ../meine-stadt-transparent && SKIP_SLOW_TESTS=1 VIRTUAL_SPRAWL=1 VIRTUAL_SPRAWL_EXTRAS="import-json" ../virtual_sprawl/.venv/bin/python -m virtual_sprawl.run pytest -p pytest_django)
# Run pytest, module
(cd ../meine-stadt-transparent && SKIP_SLOW_TESTS=1 VIRTUAL_SPRAWL=1 VIRTUAL_SPRAWL_EXTRAS="import-json" .venv/bin/python -m pytest)
