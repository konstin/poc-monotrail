...
if os.environ.get("VIRTUAL_SPRAWL"):
    ...
    try:
        sprawl_root, sprawl_packages = get_virtual_sprawl_info(sys.argv[0], get_bindings())

        class MyPathFinder(PathFinder, MetaPathFinder):
            def __init__(
                self, sprawl_root: Union[str, Path], sprawl_packages: List[Tuple[str, str]]
            ):
            ...
            def find_spec(self, fullname, path=None, target=None):
                ...
                site_packages = [
                    str(self.sprawl_root.joinpath(name + "-" + version)
                     .joinpath("lib")
                     .joinpath(
                        f"python{sys.version_info.major}.{sys.version_info.minor}"
                    )
                     .joinpath("site-packages")) for name, version in self.sprawl_packages.values()
                ]
