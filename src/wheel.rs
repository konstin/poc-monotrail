...
fn install_script(
    ...
    //
    // We do this in venvs as required, but in monorail mode we keep the original fake shebang
...
pub fn install_wheel(
    ...
    let (temp_dir_final_location, base_location) = match location {
        ...
        InstallLocation::Monorail { monorail_root, .. } => {
            let name_version_dir = monorail_root
    ...
    if data_dir.is_dir() {
        ...
        install_data(
            ...
            &gui_scripts,
            &mut record,
            // For the monorail install, we want to keep the fake shebang for our own
